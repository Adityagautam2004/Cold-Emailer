...
import { prisma, type Campaign } from "@dispatch/db";
 ...
 * `@@unique([campaignId, contactId, stepId])` constraint the spec calls out, so calling
 * this twice (e.g. two concurrent requests both passing the status check) just no-ops the
 * second write instead of duplicating sends. Extracted out of the route handler so it's
 * testable directly against real Postgres without needing an authenticated HTTP request
 * (§19 requires "campaign start creates exactly N sends" / "starting twice creates no
 * duplicates" as integration tests).
 */
export async function startCampaign(campaign: Awaited<ReturnType<typeof getOwnedCampaign>>): Promise<StartCampaignResult> {
  if (campaign.status === "running") {
    return { campaign, alreadyStarted: true };
  }
  ...
  return { campaign: updated, sendsCreated: contacts.length };
}
